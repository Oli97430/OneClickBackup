...
class DiskHealthManager:
    ...
    def run_surface_test(
        ...
        ps_script = (
            ...
            "    try {{\n"
            "      $null = $fs.Read($buf, 0, $chunkSize)\n"
            "    }} catch {{\n"
            "      $offset = [int64]$i * $chunkSize\n"
            "      $badSectors += $offset\n"
            "    }}\n"
            "    if ($i % 1000 -eq 0) {{\n"
            "      [Console]::Error.WriteLine(\"PROGRESS:$i/$totalChunks\")\n"
            "    }}\n"
            "  }}\n"
            "  $fs.Close()\n"
            "}} catch {{\n"
            "  [Console]::Error.WriteLine(\"OPENERR:$($_.Exception.Message)\")\n"
            "}}\n"
            "$result = @{{ total_sectors={total_sectors}; "
            "bad_sectors=$badSectors.Count; "
            "error_details=$badSectors }} | ConvertTo-Json -Compress\n"
            "Write-Output $result"
        )
